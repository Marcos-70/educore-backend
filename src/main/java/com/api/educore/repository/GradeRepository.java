...
public interface GradeRepository extends JpaRepository<Grade, Long> {
    ...
    @Query("SELECT g FROM Grade g JOIN FETCH g.assessment a JOIN FETCH a.subject s WHERE g.student.id = :studentId AND a.schoolClass.id = :classId AND a.trimester.id = :trimesterId")
    ...
    @Query("SELECT g FROM Grade g JOIN FETCH g.student s JOIN FETCH g.assessment a JOIN FETCH a.subject WHERE g.assessment.id = :assessmentId ORDER BY s.firstName, s.lastName")
